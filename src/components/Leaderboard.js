...
class Leaderboard extends Component {
  ...
  */
  filterRank(e) {
    ...
    for(var i = 0; i < ranking.length; i++) {
      ...
      if(str === e.target.value.toLowerCase() || str.toLowerCase().indexOf(e.target.value.toLowerCase()) > -1) {
  ...
  */
  render() {
    return (
      <div className='leaderboard' style={{"background-image": "url(./"+this.props.event+"-bg.png)", "background-size": "cover"}}>
        ...
        <div className='leaderboard-table'>
          <table id="lBoard">
            <tbody className='ranking'>
              ...
              <tr>
                <td colspan="10000">
                  <form className="input-form" onChange={this.filterRank}>
                    <input type="search" name="search" placeholder="Search your score"/>
                  </form>
                </td>
              </tr>
              <tr >
              ...
              this.state.ranking.map((user, index) => {
                return <tr className="ranking row-attendee" key={index}>
                  { user.page == this.state.page ? <td className='data'>
                    {user.rank === 1 && "🏆    "}
                    {user.rank === 2 && "🥈    "}
                    {user.rank === 3 && "🥉    "}
                  ...
                  { user.page == this.state.page ? <td className='data score'>{ user.score }</td> : null }
                </tr> }
              )
            }
            </tbody>
